...
fn value_to_log_entry(
    ...
    config: &Config,
    data: &Value,
    meta: Option<&halfbrown::HashMap<Cow<str>, Value>>,
) -> Result<LogEntry> {
    Ok(LogEntry {
        log_name: meta::log_name(config, meta),
        resource: super::value_to_monitored_resource(config.resource.as_ref())?,
        timestamp: Some(timestamp),
        receive_timestamp: None,
        severity: meta::log_severity(config, meta)?,
...
impl GclSink {
    ...
    #[cfg(test)]
    #[allow(dead_code)] // NOTE Actually used for unit testing
    pub fn set_client(
        &mut self,
...
impl Sink for GclSink {
    async fn on_event(
        ...
        for (data, meta) in event.value_meta_iter() {
            let meta = meta.get_object("gcl_writer").or(None);
        ...
        let log_entries_response = client
            .write_log_entries(WriteLogEntriesRequest {
                log_name: meta::default_log_name(&self.config),
                resource: super::value_to_monitored_resource(self.config.resource.as_ref())?,
                labels: meta::default_labels(&self.config),
                entries,
...
mod test {
    ...
    fn fails_if_the_event_is_not_an_object() -> Result<()> {
        ...
        let meta = literal!({});
        let meta = meta.get_object("gcl").or(None);
    ...
    async fn on_event_fails_if_write_stream_is_not_connected() -> Result<()> {
        ...
        assert_eq!(SinkReply::FAIL, result);
        Ok(())
    }
